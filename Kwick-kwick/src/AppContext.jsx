...
export const AppProvider = ({ children }) => {
  ...
  const addRideRequest = (ride) => {
    const newRide = {
      ...
      pickup: ride.pickupLocation?.formattedAddress || 'Current device location',
      drop: ride.destinationLocation?.formattedAddress || ride.destination || 'Destination',
  ...
  const placeOrder = (order) => {
    ...
    const newOrder = {
      ...
      drop: order.drop || 'Delivery address pending',
      dropLocation: order.dropLocation || null,
      items: order.items || [],
      total: order.total || 0,
      timestamp: new Date().toLocaleTimeString(),
      status: 'pending',
