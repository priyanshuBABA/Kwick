...
const CustomerCartEnhanced = () => {
  const { cart, removeFromCart, updateQuantity, clearCart, cartTotal, loading, error } = useCart();
  ...
  if (loading && cart.length === 0) {
    return (
      <MobileFrame>
        ...
        <div className="flex min-h-[calc(100vh-120px)] items-center justify-center p-6 text-sm font-semibold text-gray-600">
          Loading cart...
        </div>
        <BottomNav />
      </MobileFrame>
    ...
    <MobileFrame>
      ...
      <div className="pb-24">
        <div className="p-6">
          ...
          {loading && <p className="mb-4 text-sm font-semibold text-gray-600">Updating cart...</p>}
          {/* Header */}
            ...
            {cart.map((item, idx) => (
              <div key={idx} className="p-4 flex justify-between items-center hover:bg-gray-50 transition-colors group">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => updateQuantity(item.id, item.quantity - 1)} disabled={loading} aria-label={`Decrease ${item.name}`} className="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100 text-slate-700 hover:bg-slate-200"><Minus className="h-4 w-4" /></button>
                  <span className="w-5 text-center text-sm font-black">{item.quantity}</span>
                  <button onClick={() => updateQuantity(item.id, item.quantity + 1)} disabled={loading} aria-label={`Increase ${item.name}`} className="flex h-8 w-8 items-center justify-center rounded-lg bg-orange-100 text-orange-700 hover:bg-orange-200"><Plus className="h-4 w-4" /></button>
                  <button onClick={() => removeFromCart(item.id)} disabled={loading} aria-label={`Remove ${item.name}`} className="flex h-10 w-10 items-center justify-center rounded-lg bg-red-50 text-red-600 hover:bg-red-100"><Trash2 className="h-5 w-5" /></button>
                </div>
              </div>
            ))}
          </div>
