...
import { getServices } from '../../services/catalogApi';
...
const ServiceListEnhanced = () => {
  ...
  useEffect(() => {
    ...
    getServices({ limit: 20 })
      ...
      .catch((error) => {
        if (!cancelled) setServicesError(error.message || 'Unable to load services. Please try again.');
      })
      .finally(() => {
  ...
  const serviceRoute = (service) => CATEGORY_ROUTES[service.category] || '/customer/services';
    ...
    <MobileFrame>
      ...
      <div className="pb-24">
        ...
        <section className="px-6 mb-8">
          <h3 className="font-black text-lg text-slate-900 mb-4">Available Services</h3>
          {servicesLoading && <p className="rounded-2xl bg-white p-4 text-sm font-semibold text-gray-500">Loading services...</p>}
          {!servicesLoading && servicesError && <p className="rounded-2xl border border-red-100 bg-red-50 p-4 text-sm font-semibold text-red-700">{servicesError}</p>}
          {!servicesLoading && !servicesError && services.length === 0 && <p className="rounded-2xl bg-white p-4 text-sm font-semibold text-gray-500">No services available</p>}
          {!servicesLoading && !servicesError && services.length > 0 && (
            <div className="grid gap-3 sm:grid-cols-2">
              {services.map((service) => (
                <button
                  key={service.id}
                  onClick={() => navigate(serviceRoute(service))}
                  className="flex items-center gap-3 rounded-2xl border border-gray-200 bg-white p-4 text-left transition-all hover:border-orange-300 hover:shadow-md"
                >
                  <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-orange-50 text-2xl">
                    {service.image ? <img src={service.image} alt="" className="h-full w-full rounded-2xl object-cover" /> : service.emoji}
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-sm font-black text-slate-900">{service.name}</span>
                    <span className="mt-1 block truncate text-xs text-gray-500">{service.providerName || service.category}</span>
                  </span>
