...
import { Home, ShoppingBag, Grid, User, ChevronRight, MessageCircle } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { products } from '../../data/mockData';
import { useCart } from '../../CartContext';
import { useAppContext } from '../../AppContext';
// LocationPicker removed from hero; top navbar handles location
...
const CustomerHome = () => {
  ...
  React.useEffect(() => {
    // Always attempt to fetch real-time geolocation on mount
    if (!navigator.geolocation) return;

    const onSuccess = async (pos) => {
      try {
        const { latitude, longitude } = pos.coords;
        // Set a precise coords fallback immediately
        setUserLocation(`${latitude.toFixed(5)}, ${longitude.toFixed(5)}`);

        // Try reverse geocoding to get a readable address (Nominatim)
        try {
          const res = await fetch(`https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${latitude}&lon=${longitude}`);
          if (res.ok) {
            const data = await res.json();
            const display = data.display_name;
            if (display) setUserLocation(display);
          }
        } catch (err) {
      ...
      <div className="min-h-screen bg-[#F8FAFC] pb-32 relative">
        ...
        <div className="bg-[#FFD60A] pt-12 pb-20 px-6 rounded-b-[3.5rem] relative shadow-[0_30px_60px_-15px_rgba(255,214,10,0.3)] overflow-hidden">
          ...
          <div className="flex items-center gap-4 mb-8 scale-110 origin-left drop-shadow-sm">
           ...
           {/* search removed from hero - handled by top navbar */}
        </div>
