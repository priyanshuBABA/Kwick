...
const CustomerHomeEnhanced = () => {
  ...
  useEffect(() => {
    ...
    getProducts({ limit: 20 })
      .then((response) => {
        if (!cancelled) setCatalogProducts(response.data);
      })
      .catch((error) => {
        if (!cancelled) setCatalogError(error.message || 'Unable to load products. Please try again.');
      })
      .finally(() => {
  ...
  const buyAgainProducts = catalogProducts.slice(0, 2);
  const trendingProducts = catalogProducts.slice(2, 6);
          ...
          <div className="grid gap-5 xl:grid-cols-[220px_minmax(0,1fr)_330px]">
            ...
            <section className="min-w-0">
              ...
              <div className="mb-5">
                ...
                <p className="mt-2 text-sm text-slate-500">{formattedAddress ? `Delivering to ${formattedAddress}` : 'Select your location to get started'}</p>
              </div>
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                <div className="rounded-[28px] border border-slate-200 bg-white p-4 shadow-sm">
                  ...
                  <div className="space-y-3">
                    {catalogMessage && <p className="rounded-2xl bg-slate-50 p-4 text-sm font-semibold text-slate-500">{catalogMessage}</p>}
