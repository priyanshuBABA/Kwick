...
const AmbulanceService = () => {
  ...
  const [bookingData, setBookingData] = useState({ service: null, pickup: '', dropMode: 'nearest', hospital: null, medInfo: '', condition: '' });
  ...
  useEffect(() => {
    if (formattedAddress) setBookingData((prev) => ({ ...prev, pickup: formattedAddress, pickupLocation: currentLocation }));
  ...
  const handleConfirmBooking = () => {
    if (!currentLocation || (dropMode === 'choose' && !selectedHospital)) return;
    setFlow('tracking');
  };
  ...
  const resetFlow = () => {
    ...
    setEta(7);
    setCopied(false);
    setBookingData({ service: null, pickup: '', dropMode: 'nearest', hospital: null, medInfo: '', condition: '' });
  };
    ...
    <div style={{ padding: '16px 16px 120px', background: COLORS.bg }}>
      <div style={{ background: COLORS.card, borderRadius: 20, padding: 16, marginBottom: 12, boxShadow: '0 10px 30px rgba(0,0,0,0.05)' }}>
        ...
        <div style={{ background: 'rgba(255,215,0,0.14)', border: '1px solid rgba(255,215,0,0.3)', borderRadius: 16, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ fontSize: 14, fontWeight: 800 }}>{locationLoading ? 'Detecting device location...' : bookingData.pickup || 'Current location unavailable'}</div>
        </div>
        {locationError && <div style={{ color: COLORS.accent, fontSize: 11, fontWeight: 700, marginTop: 8 }}>{locationError}</div>}
      ...
      <div style={{ position: 'fixed', left: 0, right: 0, bottom: 0, background: '#fff', borderTop: '1px solid #eee', padding: '14px 16px 18px', zIndex: 90 }}>
        <button onClick={handleConfirmBooking} disabled={!currentLocation || (dropMode === 'choose' && !selectedHospital)} style={{ width: '100%', border: 'none', borderRadius: 16, background: !currentLocation || (dropMode === 'choose' && !selectedHospital) ? '#d3d3d3' : COLORS.accent, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '14px 16px', fontSize: 15, fontWeight: 900, letterSpacing: '0.08em', textTransform: 'uppercase', cursor: !currentLocation || (dropMode === 'choose' && !selectedHospital) ? 'not-allowed' : 'pointer' }}>
