...
import { products, offers } from '../data/mockData';
...
const SEARCH_PRODUCTS = [
  ...(products?.freshMandi || []).map(p => ({ title: p.name, icon: p.emoji, category: 'Fresh Mandi', path: '/customer/fresh-mandi', price: `₹${p.price} / ${p.weight}` })),
  ...(products?.foodCakes || []).map(p => ({ title: p.name, icon: p.emoji, category: 'Cakes & Food', path: '/customer/mishra-ji-cakes', price: `₹${p.price}` })),
  ...(products?.medicines || []).map(p => ({ title: p.name, icon: p.emoji, category: 'Medicines', path: '/customer/medicines', price: `₹${p.price}` })),
  ...(products?.household || []).map(p => ({ title: p.name, icon: p.emoji, category: 'Household', path: '/customer/household-items', price: `₹${p.price}` })),
  ...(products?.services || []).map(p => ({ title: p.name, icon: p.emoji, category: 'Services', path: '/customer/home-services', price: `₹${p.price}` })),
];
...
const CustomerTopNav = ({ onMenuClick, searchQuery, onSearchQueryChange }) => {
  ...
  const matchingProducts = trimmedQuery
    ? SEARCH_PRODUCTS.filter(p => `${p.title} ${p.category}`.toLowerCase().includes(trimmedQuery))
    : [];
