...
function formatCoordinates(location, unavailableLabel) {
...
function getPickupLocations(order) {
  if (Array.isArray(order?.pickupLocations) && order.pickupLocations.length) return order.pickupLocations
  return order?.pickupLocation ? [order.pickupLocation] : []
}
...
function Home({ online, setOnline, stats, availableOrders, assignedOrders, activeOrder, goToOrders, rider, loc, onAccept, acceptingOrderId }) {
  const initials = (rider.fullName || 'Rider').split(' ').map((w) => w[0]).join('').slice(0, 2).toUpperCase();
  const nextAvailableOrder = availableOrders[0];
  const activeAssignedOrder = assignedOrders.find((order) => ['rider_assigned', 'picked_up', 'out_for_delivery'].includes(order.status)) || null;
  ...
  return (
    <div className="px-4 pt-5 pb-28 space-y-5 w-full">
      ...
      {online && (
        <div className="flex items-center gap-2 rounded-xl px-4 py-3" style={{ background: C.green2 }}>
          ...
          <p className="text-sm font-medium" style={{ color: C.green }}>
            {availableOrders.length > 0 ? `${availableOrders.length} available delivery orders in your queue` : 'No available orders right now — stay online for the next pickup'}
          </p>
        </div>
      )}
      ...
      {(activeAssignedOrder || (online && nextAvailableOrder && !activeOrder)) && (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-sm" style={{ color: C.text }}>
                  {activeAssignedOrder ? `Ongoing: ${activeAssignedOrder.status}` : `Available: #${String(nextAvailableOrder._id).slice(-6)}`}
                </p>
                <p className="text-xs" style={{ color: C.textDim }}>{activeAssignedOrder ? 'Tap to continue trip' : 'Tap to review and accept order'}</p>
              </div>
            </div>
            <ChevronRight size={18} color={C.gold} />
          </div>
          {!activeAssignedOrder && nextAvailableOrder && (
            <Button variant="primary" className="w-full mt-3" onClick={() => onAccept(nextAvailableOrder._id)} disabled={acceptingOrderId === nextAvailableOrder._id}>
              {acceptingOrderId === nextAvailableOrder._id ? 'Accepting…' : 'Accept this order'}
            </Button>
          )}
        </Card>
      )}
...
function Orders({
  ...
  const orderedList = [...assignedOrders, ...availableOrders];
  const filtered = orderedList.filter((order) => {
    const matchesFilter = filter === 'All' ||
      (filter === 'Active' && ['rider_assigned', 'picked_up', 'out_for_delivery'].includes(order.status)) ||
      (filter === 'Completed' && order.status === 'delivered') ||
      (filter === 'Cancelled' && order.status === 'cancelled');
    const haystack = [order._id, order.status, order.userId || '', order.shippingAddress?.city || '']
      .filter(Boolean)
      .join(' ')
      .toLowerCase();
  ...
  const getNextStatus = (status) => {
    const map = { rider_assigned: 'picked_up', picked_up: 'out_for_delivery', out_for_delivery: 'delivered' };
  ...
  if (loading) {
    return (
      <div className="px-4 pt-5 pb-28 space-y-4 w-full">
        <p className="text-sm font-medium" style={{ color: C.text }}>Loading rider orders…</p>
      </div>
    );
  }
    ...
    return (
      <div className="px-4 pt-5 pb-28 space-y-4 w-full">
        <Card style={{ padding: 22 }}>
          <p role="alert" style={{ color: C.red }}>{error}</p>
        </Card>
      </div>
    );
  }
    ...
    return (
      <div className="px-4 pt-5 pb-28 space-y-4 w-full">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-extrabold" style={{ color: C.text }}>Order #{String(order._id || order.id).slice(-6)}</h1>
        </div>

        <Card style={{ padding: 16 }}>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2"><span className="text-xl">📦</span><p className="font-semibold text-sm" style={{ color: C.text }}>Delivery task</p></div>
            <Badge tone={order.status === 'delivered' ? 'green' : order.status === 'rider_assigned' ? 'gold' : 'dim'}>{order.status}</Badge>
          </div>
          <InfoRow label="Status" value={order.status} />
          <InfoRow label="Customer" value={order.userId || 'Customer'} />
          <div className="py-2.5" style={{ borderBottom: `1px solid ${C.border}` }}>
            <p className="text-sm" style={{ color: C.textMid }}>Pickup locations</p>
            <div className="mt-2 space-y-2">
              {getPickupLocations(order).length ? getPickupLocations(order).map((location) => (
                <div key={location.vendorId || location.address} className="text-right">
                  <p className="text-sm font-semibold" style={{ color: C.text }}>{location.businessName || 'Vendor pickup'}</p>
                  <p className="text-xs" style={{ color: C.textMid }}>{location.address}, {location.city}</p>
                  <p className="text-xs" style={{ color: C.textMid }}>{formatCoordinates(location, 'Pickup coordinates unavailable')}</p>
                </div>
              )) : <p className="text-sm font-semibold text-right" style={{ color: C.text }}>Vendor pickup location unavailable</p>}
            </div>
          </div>
          ...
          {nextStatus && (
            <Button
              ...
              onClick={() => onAdvanceStatus(order._id || order.id, nextStatus)}
              disabled={statusUpdatingOrderId === (order._id || order.id)}
            >
              {statusUpdatingOrderId === (order._id || order.id) ? 'Updating…' : `Move to ${nextStatus}`}
            </Button>
          )}
        </Card>
      </div>
  ...
  return (
    <div className="px-4 pt-5 pb-28 space-y-4 w-full">
      ...
      <div className="flex items-center gap-2 px-4 py-3 rounded-xl" style={{ background: C.surface3, border: `1px solid ${C.border}` }}>
        ...
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search by order ID or city" className="flex-1 bg-transparent outline-none text-sm" style={{ color: C.text }} />
      </div>

      <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
        {['All', 'Active', 'Completed', 'Cancelled'].map((f) => (
          <button key={f} onClick={() => setFilter(f)} className="px-4 py-2 rounded-xl text-sm font-semibold whitespace-nowrap flex-shrink-0 transition-all" style={{ background: filter === f ? `linear-gradient(135deg, ${C.gold}, ${C.gold2})` : C.surface3, color: filter === f ? '#241A05' : C.textMid, border: `1px solid ${filter === f ? 'transparent' : C.border}` }}>
            {f}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-3">
        {filtered.length === 0 && (
          <Card style={{ padding: 30 }} className="text-center">
            ...
            <p className="text-xs mt-1" style={{ color: C.textDim }}>Available and assigned deliveries will appear here.</p>
          </Card>
        )}
        {filtered.map((order) => {
          const canAccept = order.status === 'placed' && !order.assignedRiderId;
          const canAdvance = ['picked_up', 'out_for_delivery'].includes(order.status) || (order.status === 'rider_assigned' && order.vendorStatus === 'ready');
          return (
            <Card key={order._id || order.id} style={{ padding: 16 }} className="cursor-pointer active:scale-[0.98] transition-transform">
              <div onClick={() => setOpenOrder(order)} className="flex items-center justify-between mb-3">
                <div>
                  <p className="font-bold text-sm" style={{ color: C.text }}>#{String(order._id || order.id).slice(-6)}</p>
                  <p className="text-xs" style={{ color: C.textDim }}>{order.shippingAddress?.city || 'Address pending'}</p>
                </div>
                <Badge tone={order.status === 'delivered' ? 'green' : order.status === 'rider_assigned' ? 'gold' : 'dim'}>{order.status}</Badge>
              </div>
              <div className="flex items-center justify-between text-xs" style={{ color: C.textDim }}>
                <span>{order.items?.length || 0} items</span>
                <span className="text-sm font-bold" style={{ color: C.gold2 }}>₹{Number(order.total || 0)}</span>
              </div>
              <div className="mt-3 space-y-1 text-xs" style={{ color: C.textMid }}>
                {getPickupLocations(order).length ? getPickupLocations(order).map((location) => (
                  <div key={location.vendorId || location.address}>
                    <p><span className="font-semibold">PICKUP {location.businessName ? `(${location.businessName})` : ''}:</span> {location.address}, {location.city}</p>
                    <p><span className="font-semibold">Pickup coordinates:</span> {formatCoordinates(location, 'Pickup coordinates unavailable')}</p>
                  </div>
                )) : <p><span className="font-semibold">PICKUP:</span> Vendor pickup location unavailable</p>}
                <p><span className="font-semibold">DELIVERY:</span> {order.deliveryAddress || order.shippingAddress?.formattedAddress || order.shippingAddress?.city || 'Address pending'}</p>
                <p><span className="font-semibold">Delivery coordinates:</span> {formatCoordinates(order.shippingAddress, 'Delivery coordinates unavailable')}</p>
              </div>
              <div className="flex gap-2 mt-3">
                {canAccept && (
                  <>
                    <Button variant="primary" className="flex-1" onClick={() => onAccept(order._id || order.id)} disabled={statusUpdatingOrderId === (order._id || order.id)}>
                      {statusUpdatingOrderId === (order._id || order.id) ? 'Accepting…' : 'Accept'}
                    </Button>
                    <Button variant="danger" className="flex-1" onClick={() => onDecline(order._id || order.id)} disabled={statusUpdatingOrderId === (order._id || order.id)}>
                      Decline
                    </Button>
                  </>
                )}
                {!canAccept && canAdvance && (
                  <Button variant="success" className="flex-1" onClick={() => onAdvanceStatus(order._id || order.id, getNextStatus(order.status))} disabled={statusUpdatingOrderId === (order._id || order.id)}>
                    {statusUpdatingOrderId === (order._id || order.id) ? 'Updating…' : `Next: ${getNextStatus(order.status)}`}
                  </Button>
                )}
              </div>
            </Card>
...
export default function KwickRiderPage() {
  ...
  const loc = {
    status: locationLoading ? 'loading' : currentLocation ? 'granted' : 'idle',
    label: formattedAddress,
    error: locationError,
    detect: refreshLocation,
  };

  const stats = { todayRides: assignedOrders.length, vsYesterday: 0, efficiency: 92, lifetimeOrders: 0, lifetimeEarnings: 0 };

  const loadRiderOrders = React.useCallback(async () => {
    if (!token) return;
    setOrdersLoading(true);
    setOrdersError('');
    try {
      const [available, assigned] = await Promise.all([
        getAvailableRiderOrders(token),
        getMyRiderOrders(token),
      ]);
      setAvailableOrders(available);
      setAssignedOrders(assigned);
      const currentActiveOrder = assigned.find((order) => ['rider_assigned', 'picked_up', 'out_for_delivery'].includes(order.status)) || null;
      setActiveOrder(currentActiveOrder);
      setPendingOrder(online && !currentActiveOrder && available[0] ? available[0] : null);
    } catch (error) {
      setOrdersError(error.message || 'Unable to load rider orders');
      setAvailableOrders([]);
      setAssignedOrders([]);
    } finally {
  ...
  const finishOnboarding = (data) => { setRider(data); setStage('app'); };

  const handleAcceptOrder = async (orderId) => {
    if (!token || !orderId) return;
    try {
      setAcceptingOrderId(orderId);
      const acceptedOrder = await acceptRiderOrder(orderId, token);
      setActiveOrder(acceptedOrder);
      setPendingOrder(null);
      setTab('orders');
      await loadRiderOrders();
    } catch (error) {
      setOrdersError(error.message || 'Unable to accept order');
    } finally {
  ...
  const handleDeclineOrder = async (orderId) => {
    ...
    try {
      ...
      await declineRiderOrder(orderId, token);
      await loadRiderOrders();
    } catch (error) {
      setOrdersError(error.message || 'Unable to decline order');
    } finally {
  ...
  const handleAdvanceStatus = async (orderId, nextStatus) => {
    ...
    try {
      setStatusUpdatingOrderId(orderId);
      const updatedOrder = await updateRiderOrderStatus(orderId, nextStatus, token);
      setActiveOrder(updatedOrder);
      await loadRiderOrders();
    } catch (error) {
      setOrdersError(error.message || 'Unable to update order status');
    } finally {
      setStatusUpdatingOrderId(null);
    }
  };
  ...
  const handleLogout = () => {
    ...
    setActiveOrder(null); setAvailableOrders([]); setAssignedOrders([]); setRider({});
  ...
  return (
    <div className="min-h-screen w-full" style={{ background: C.bg, fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      <div style={{ width: '100%', minHeight: '100vh', position: 'relative', paddingBottom: '80px' }}>
        ...
        {pendingOrder && (
          <OrderRequestPopup order={{ ...pendingOrder, service: { icon: '📦', name: 'Delivery', cat: 'Delivery' }, pickup: pendingOrder.shippingAddress?.city || 'Pickup', drop: pendingOrder.shippingAddress?.city || 'Drop', distance: 'N/A', payout: Number(pendingOrder.total || 0) }} onAccept={() => handleAcceptOrder(pendingOrder._id)} onReject={() => setPendingOrder(null)} />
        )}
      </div>
    </div>
  );
